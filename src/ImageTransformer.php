...
class ImageTransformer
{
     ...
     * 验证翻转方向参数
     */
    private function validateFlipDirection(string $direction): void
    {
        if (self::FLIP_HORIZONTAL !== $direction && self::FLIP_VERTICAL !== $direction) {
     ...
     * 获取图像尺寸
     *
     * @return array{0: int, 1: int} 返回包含宽度和高度的数组
     */
     ...
     * 执行翻转操作
     *
     * @param array<string, array<int, array<int, int>>> $channels 通道数据
     * @param string                                     $direction 翻转方向
     * @param int                                        $width     图像宽度
     * @param int                                        $height    图像高度
     *
     * @return array<string, array<int, array<int, int>>> 翻转后的通道数据
     */
    private function performFlip(array $channels, string $direction, int $width, int $height): array
    {
        if (self::FLIP_HORIZONTAL === $direction) {
            return $this->flipHorizontal($channels, $height);
        }

        return $this->flipVertical($channels);
    }
     ...
     * @return array<string, array<int, array<int, int>>> 翻转后的通道数据
     */
    private function flipHorizontal(array $channels, int $height): array
    {
     ...
     * @return array<string, array<int, array<int, int>>> 翻转后的通道数据
     */
    private function flipVertical(array $channels): array
    {
        foreach (['red', 'green', 'blue'] as $channel) {
    ...
    public function rotate(float $angle): bool
    {
        ...
        if (abs($normalizedAngle - 90) < 0.001) {
            $channels = $this->rotateChannels90($channels);
        } elseif (abs($normalizedAngle - 180) < 0.001) {
            $channels = $this->rotateChannels180($channels);
        } elseif (abs($normalizedAngle - 270) < 0.001) {
            $channels = $this->rotateChannels270($channels);
        } else {
     ...
     * @param array<string, array<int, array<int, int>>> $channels 通道数据
     *
     * @return array<string, array<int, array<int, int>>> 旋转后的通道数据
     */
    protected function rotateChannels90(array $channels): array
    {
     ...
     * @return array<string, array<int, array<int, int>>> 旋转后的通道数据
     */
    protected function rotateChannels180(array $channels): array
    {
        foreach (['red', 'green', 'blue'] as $channel) {
            // 水平翻转每一行
            foreach ($channels[$channel] as $index => $row) {
                $channels[$channel][$index] = array_reverse($row);
            }
     ...
     * @return array<string, array<int, array<int, int>>> 旋转后的通道数据
     */
    protected function rotateChannels270(array $channels): array
    {
        $height = count($channels['red']);
        $width = count($channels['red'][0]);
